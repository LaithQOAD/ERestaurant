...
{
    public sealed class OrderService : IOrderService, IOrderPricingService
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;
        private readonly IRequestLanguage _language;

        public OrderService(IUnitOfWork unitOfWork, IMapper mapper, IRequestLanguage language)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
            _language = language;
        }
        public async Task<List<OrderDTO>> FindAllAsync(FindAllOrderParameterDTO param)
        {
            var query = _unitOfWork.Order.Query();

            if (!string.IsNullOrWhiteSpace(param.searchQuery))
            {
                var customerNameSearch = param.searchQuery.Trim();
                query = query.Where(o =>
                    o.CustomerName != null &&
                    EF.Functions.Like(o.CustomerName, $"%{customerNameSearch}%"));
            }

            if (param.dateFrom != default)
            {
                var startDate = new DateTimeOffset(param.dateFrom.Date, TimeSpan.Zero);
                query = query.Where(o => o.OrderDate >= startDate);
            }

            if (param.dateTo != default)
            {
                var endDateExclusive = new DateTimeOffset(param.dateTo.Date.AddDays(1), TimeSpan.Zero);
                query = query.Where(o => o.OrderDate < endDateExclusive);
            }

            if (param.isActive.HasValue)
                query = query.Where(o => o.IsActive == param.isActive.Value);

            query = ApplyOrdering(query, param.orderBy, param.orderByDirection);
            ...
            return _mapper.Map<List<OrderDTO>>(
                pagedOrders, opt => opt.Items["isArabic"] = _language.IsArabic);
        }
